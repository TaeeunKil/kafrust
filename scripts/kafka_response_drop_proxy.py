#!/usr/bin/env python3
"""Forward Kafka traffic and drop the first response for one API key."""
...
async def proxy_connection(
    ...
    async def forward_responses() -> None:
        while True:
            ...
            if api_key == drop_api_key and claim_drop_marker(drop_marker):
                print(
                    f"forwarded API key {api_key} request and dropped its response",
                    flush=True,
                )
                return
...
async def main() -> None:
    ...
    parser.add_argument("--drop-api-key", type=int, required=True)
