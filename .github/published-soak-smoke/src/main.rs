...
async fn main() -> kafrust::Result<()> {
    ...
    let mut producer = ProducerConfig::new([bootstrap_servers.clone()])
        .client_id("kafrust-published-soak-producer")
    ...
    let mut consumer = ConsumerConfig::new([bootstrap_servers])
        .client_id("kafrust-published-soak-consumer")
        .metrics(metrics.clone())
        .max_wait_ms(100)
        .max_retries(3)
    ...
    while Instant::now() < deadline {
        ...
        let metadata = match producer.send_batch(records).await {
            ...
            Err(error) => {
                eprintln!("published soak produce operation failed: {error}");
                operation_errors += 1;
                saw_error = true;
        ...
        let Some(first) = metadata.first() else {
            return Err(kafrust::Error::Unsupported(
                "published soak producer returned no metadata",
            ));
        };
        let mut next_offset = first.offset();
        if next_offset < 0 {
            return Err(kafrust::Error::Unsupported(
                "published soak requires concrete broker offsets",
            ));
        }
        ...
        while remaining > 0 {
            if Instant::now() >= hard_deadline {
                return Err(kafrust::Error::Unsupported(
                    "published soak consumer did not drain before the deadline",
                ));
            }
            match consumer.fetch(topic.clone(), 0, next_offset).await {
                ...
                Err(error) => {
                    eprintln!("published soak fetch operation failed: {error}");
                    operation_errors += 1;
                    saw_error = true;
                    tokio::time::sleep(RECOVERY_BACKOFF).await;
                }
    ...
    if produced != consumed || snapshot.produced_records != snapshot.consumed_records {
        return Err(kafrust::Error::Unsupported(
            "published soak finished with unmatched produced and consumed records",
        ));
    }
    if snapshot.in_flight_requests != 0 || snapshot.buffered_records != 0 {
        return Err(kafrust::Error::Unsupported(
            "published soak finished with non-zero client gauges",
        ));
    }
    if !saw_error || !recovered_after_error {
        return Err(kafrust::Error::Unsupported(
            "published soak did not observe both an error and recovery",
        ));
    }
