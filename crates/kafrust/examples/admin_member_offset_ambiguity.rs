...
async fn main() -> kafrust::Result<()> {
    ...
    let admin_config = common::apply_security(
        ClientConfig::new(bootstrap_servers.clone())
            .client_id("kafrust-admin-member-offset-ambiguity"),
    )?;
    ...
    let group_config = common::apply_security(
        ConsumerGroupConfig::new(bootstrap_servers, group_id.clone())
            .client_id("kafrust-admin-member-offset-ambiguity-member")
    ...
    ensure_success(
        before.error_code(),
        "list member-aware offsets before commit",
    )?;
    println!(
            ...
            .await
        {
            Ok(result) if result.is_success() => {
                return Err(Error::Unsupported(
                    "the response-drop proxy did not make member-aware OffsetCommit ambiguous",
                ))
            }
            Ok(_) => return Err(Error::Unsupported(
                "member-aware OffsetCommit returned a broker error before the response was dropped",
            )),
            Err(error) => error,
        };
    if !matches!(
    ...
    println!("member-aware OffsetCommit response was lost; outcome is explicitly unknown");
    let after = admin
        .list_consumer_group_offsets_with_member(
            &group_id,
            Some(&member_id),
    ...
    if committed != target_offset {
        return Err(Error::Broker {
            code: -1,
            context: format!(
                "member-aware OffsetCommit reconciliation expected {target_offset}, received {committed}"
            ),
        });
    }
    println!(
        "reconciled member-aware offset for {group_id}/{topic}-{partition} offset={committed}"
    );
    group.leave().await?;
    Ok(())
}
