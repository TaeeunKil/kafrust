...
async fn main() -> kafrust::Result<()> {
    ...
    let mut consumer = common::apply_security(
        ConsumerConfig::new(bootstrap_servers).client_id("kafrust-consumer-failover-example"),
    )?
    .max_retries(5)
    .build()
    .await?;

    let before = consumer.fetch(topic.clone(), partition, offset).await?;
    ...
    let after = consumer.fetch(topic, partition, offset).await?;
