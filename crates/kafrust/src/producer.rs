...
impl ProducerConfig {
    ...
    /// producer. A value of zero is rejected during validation.
...
mod tests {
    ...
    #[test]
    fn selects_flexible_produce_v9_when_available() {
        let versions = api_versions(9);
        let record = ProducerRecord::to("orders").header("source", "checkout");
    ...
    #[test]
    fn selects_flexible_produce_v9_for_batch_when_available() {
        let versions = api_versions(9);
        let first = BatchRecord::new(ProducerRecord::to("orders").header("source", "checkout"));
        let batch = [first];
        let records = prepared_records(&batch);
    ...
    #[tokio::test]
    async fn retries_idempotent_batch_after_partial_produce_write_with_same_sequence() {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        let retry_server = tokio::spawn(async move {
            let (mut socket, _) = listener.accept().await.unwrap();
        ...
        broker_clients
            .insert(
                ...
                Client::from_stream(
                    Box::new(PartialWriteAfterFirst::new(partial_stream, 3)),
                    Some("kafrust-partial-batch-produce-test".to_owned()),
                    Some(Duration::from_millis(500)),
                ),
                1,
            )
        ...
        let mut producer = Producer {
            client: Client::from_stream(
                Box::new(bootstrap_stream),
                Some("kafrust-producer-partial-batch-write-test".to_owned()),
                Some(Duration::from_millis(500)),
            ),
            config,
            metadata_cache,
        ...
        let metadata = producer
            .send_batch([ProducerRecord::to("orders").partition(0).value("created")])
            .await
            .unwrap();
