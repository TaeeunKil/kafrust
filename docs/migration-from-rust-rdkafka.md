...
| Published simultaneous broker-loss soak | Candidate for the tested plaintext topology; the published `0.2.28` artifact ran a fresh external Kafka 4.3.1 three-broker workload through simultaneous ten-second outages of brokers 1 and 2, reconciled 4,423,200 records, and ended with zero in-flight/buffered records in [`Published Multi-Broker Soak Smoke` run `31748293446`](https://github.com/TaeeunKil/kafrust/actions/runs/31748293446). Qualify the target service's secured simultaneous-loss behavior, concurrent clients, deploy/restart behavior, and rollback path |
