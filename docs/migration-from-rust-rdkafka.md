...
| Consumer-group listing | Candidate; high-level `list_groups` queries every advertised broker, negotiates ListGroups v4/v5 when available, deduplicates and sorts group listings, and preserves protocol type, coordinator ID, throttle time, state, type, API version, and broker errors. `list_groups_with_options` adds v4/v5 state/type filters; the low-level `Client::list_groups_v1` path remains available for exact legacy compatibility. The live smoke gate now asserts v4 negotiation on Kafka 3.7.2 and v5 state/type-filter negotiation on Kafka 4.3.1; a successful workflow run is required before counting the modern path as released evidence. In-flight broker-stop recovery restarts the broker during the bounded reconnect loop and records `retries=7` on the Kafka 3.7.2 three-broker profile in [`Live Kafka Smoke` run `31616181960`](https://github.com/TaeeunKil/kafrust/actions/runs/31616181960), and active-group listing is verified from the published `0.2.28` artifact across classic and KIP-848 profiles in [`Published Crate Smoke` run `31737581786`](https://github.com/TaeeunKil/kafrust/actions/runs/31737581786); qualify target authorization and behavior while a coordinator broker remains unavailable |
