...
| Record deletion | Candidate; DeleteRecords v1 is leader-routed, retries fixed-offset operations after transient transport or leader movement, and preserves low watermarks and per-partition errors. An in-flight leader-stop recovery is live-verified on the Kafka 3.7.2 three-broker profile in [`Live Kafka Smoke` run `31607006237`](https://github.com/TaeeunKil/kafrust/actions/runs/31607006237); qualify retention policy and destructive-operation controls on the target broker |
