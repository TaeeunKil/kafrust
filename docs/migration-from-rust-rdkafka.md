...
| Finalized feature updates | `AdminClient::update_features()` | Routes UpdateFeatures v0 to the active controller and preserves top-level/per-feature outcomes. Treat `AdminMutationOutcomeUnknown` as an ambiguous controller mutation and inspect feature metadata before retrying. |
