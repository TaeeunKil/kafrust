...
Enable `kafrust`'s `tls` feature for TLS or SASL over TLS. Async kafrust APIs
require Tokio. The optional `blocking` feature owns a dedicated multi-thread
Tokio runtime for synchronous Producer, manually assigned direct Consumer,
joined Consumer Group, Share Consumer, Streams session, and broad
broker/controller Admin calls; those adapters must be used outside any
existing Tokio runtime. All currently exposed Share Group State Admin calls are
also available synchronously; a general alternate-runtime abstraction remains
open.
...
| shared bootstrap, security, limits, controller, or metrics policy | `ClientConfig` plus `*.with_client_config(...)` on producer, direct consumer, consumer group, share consumer, or Streams group configuration | Build one validated client policy and inject it into each high-level client. Per-builder setters remain available for small local overrides. |
| connection-free configuration validation | `*.build_config()` on producer, direct consumer, consumer group, share consumer, Streams group configuration, or `AdminClient::new(...).build_config()` | Validate settings before starting an async runtime or opening a broker connection; the returned value can then be passed to the matching async `build()` or `join()` path. |
...
| `delivery.timeout.ms` | `.delivery_timeout_ms(...)` on `ProducerConfig` | Applies one total deadline from enqueue through Produce for immediate, batch, and buffered deliveries, including metadata, retries, and backoff. Defaults to 120 seconds; `linger_ms` controls batching latency independently. |
...
| TLS server roots and mutual TLS | `.tls_server_name(...)`, `.tls_root_certificate_der(...)`, `.tls_client_certificate_der(...)`, `.tls_client_private_key_der(...)` on `ClientConfig` or the matching high-level builder | Client certificates and private keys are DER inputs, must be configured as a pair, and require `SecurityProtocol::Tls` or `SecurityProtocol::SaslTls`; the private key is redacted from `Debug`. Live mTLS qualification is still open. |
...
| `sasl.mechanism=OAUTHBEARER` | `.sasl_oauthbearer(token)`, `.sasl_oauthbearer_with_username(username, token)`, or the matching `*_provider` builder | Live verified against Kafka 3.7.2's built-in validator and a signed local OIDC/JWKS fixture, including Java client, static-token, and provider-backed paths in [`Live Kafka Smoke` OIDC job 31584760474`](https://github.com/TaeeunKil/kafrust/actions/runs/31584760474/job/94078116567). Initial OAUTHBEARER authentication uses flexible `SaslAuthenticate v2`; provider re-authentication uses `v1`; external provider-specific behavior remains open. |
...
| Kafka feature metadata | `AdminClient::describe_features()` | Reads broker-supported and cluster-finalized feature ranges from ApiVersions v3 tagged fields, including the finalized epoch. Qualify the returned feature set against the target Kafka version and controller policy. |
| Finalized feature updates | `AdminClient::update_features()` | Routes UpdateFeatures v1 to the active controller with v0 fallback and preserves top-level/per-feature outcomes. Current-source Kafka 3.7.2 and 4.3.1 gates validate a non-empty `metadata.version` request in [`32360936437`](https://github.com/TaeeunKil/kafrust/actions/runs/32360936437) and [`32361035007`](https://github.com/TaeeunKil/kafrust/actions/runs/32361035007). The SASL/PLAIN authorization matrix [`32362301496`](https://github.com/TaeeunKil/kafrust/actions/runs/32362301496) confirms restricted-user rejection and administrator success; the controller-failover matrix [`32363072430`](https://github.com/TaeeunKil/kafrust/actions/runs/32363072430) confirms validation after leader replacement; Kafka 4.3.1 lifecycle [`32363428806`](https://github.com/TaeeunKil/kafrust/actions/runs/32363428806) confirms `transaction.version` `2 -> 1 -> 2`. Treat `AdminMutationOutcomeUnknown` as an ambiguous controller mutation and inspect feature metadata before retrying; metadata-version transitions and state-changing failover qualification remain open. |
...
`BufferedProducer`; call `BufferedProducer::handle` to obtain a cloneable
`BufferedProducerHandle` for non-transactional sends, and keep each returned
delivery handle for per-record completion. Stop using the handles before the
owning buffered producer is closed. Transactional buffered producers keep
enqueueing on the owner so transaction boundaries remain ordered.
...
When Kafka returns a retryable broker error with both `CurrentLeader` and
`NodeEndpoints`, kafrust uses that complete endpoint hint for the next produce
attempt; incomplete hints fall back to metadata refresh.
...
registration prefers flexible `AddPartitionsToTxn` and `AddOffsetsToTxn` v3
after ApiVersions negotiation, with v0 fallback for Kafka 3.7-era brokers.
...
`alter_consumer_group_offsets_with_member`; these use OffsetFetch/OffsetCommit
v10 when the coordinator advertises v10 and Metadata v12 resolves topic names;
callers can attach complete topic UUIDs with the public query/offset builders
to skip discovery. Otherwise they fall back to v9.
The current member epoch is preserved and stale-epoch errors remain typed.
...
replayed. The four-job Kafka 3.7.2, 3.8.1, 3.9.1, and 4.3.1 three-node KRaft
matrix then stopped broker 1, unregistered it through the surviving controller
quorum, restarted the same node, and verified re-registration plus quorum
...
| Published `kafrust 0.3.1` versus rust-rdkafka performance comparison | A fresh external project compared crates.io `kafrust 0.3.1` with `rust-rdkafka 0.39.0` on Kafka 4.3.1 using 20,000 1-KiB records in batches of 200. Kafrust measured 54,613.98 producer records/s and 256,404.94 consumer records/s; rust-rdkafka measured 91,577.21 producer records/s and 323,988.79 consumer records/s in [`Published rust-rdkafka Comparison`, run `32355261735`](https://github.com/TaeeunKil/kafrust/actions/runs/32355261735). The run also confirms the published Fetch v13 schema-order fix; this is one baseline only, not API/feature parity or a production SLO; repeat with the target service's payloads, concurrency, compression, and broker topology |
| Repeated published `kafrust 0.3.1` versus rust-rdkafka performance comparison | A fresh external project repeated the same Kafka 4.3.1, 20,000-record, 1-KiB, batch-200 profile three times per implementation on independent topics. Kafrust ranged from 45,767.55 to 61,927.71 producer records/s and 217,064.20 to 292,880.32 consumer records/s; rust-rdkafka ranged from 86,085.93 to 165,229.42 producer and 207,251.77 to 795,296.43 consumer records/s in [`Published rust-rdkafka Comparison`, run `32368443357`](https://github.com/TaeeunKil/kafrust/actions/runs/32368443357). The spread shows why this is a workload baseline rather than a universal performance ranking; repeat with the target service's payloads, concurrency, compression, and broker topology |
| Published `kafrust 0.3.5` versus rust-rdkafka performance comparison | A fresh external project compared crates.io `kafrust 0.3.5` with `rust-rdkafka 0.39.0` on Kafka 4.3.1 using three independent repetitions of 20,000 1-KiB records in batches of 200. Kafrust measured median 65,451.79 producer and 351,376.54 consumer records/s; rust-rdkafka measured median 162,827.84 producer and 615,755.30 consumer records/s in [`Published rust-rdkafka Comparison`, run `32432679837`](https://github.com/TaeeunKil/kafrust/actions/runs/32432679837). This remains historical artifact evidence; the current 0.3.6 comparison is recorded below and neither run is API/feature parity, failure compatibility, or a production SLO. |
| Published `kafrust 0.3.6` versus rust-rdkafka performance comparison | A fresh external project compared crates.io `kafrust 0.3.6` with `rust-rdkafka 0.39.0` on Kafka 4.3.1 using three independent repetitions of 20,000 unique 1-KiB records in batches of 200. All rows had zero loss/duplicates and a matching payload digest. Kafrust measured median 76,625.07 producer and 272,659.38 consumer records/s; rust-rdkafka measured median 153,296.60 producer and 590,466.70 consumer records/s in [`Published rust-rdkafka Comparison`, run `32619987006`](https://github.com/TaeeunKil/kafrust/actions/runs/32619987006). This workload-specific result is informational, not API/feature parity or a production SLO; target-service profiling and the V1-22 SLO gates remain required. |
...
| Published `0.3.3` simultaneous multi-broker broker-restart soak | Candidate for the tested current published plaintext topology; the published `0.3.3` artifact ran a fresh external 120-second Kafka 4.3.1 three-broker workload with three replicated partitions through a simultaneous ten-second outage of brokers 1 and 2, reconciled 4,099,200 records, observed 1 operation error and 1,018 retries, and ended with zero in-flight/buffered records in [`Published Multi-Broker Soak Smoke` run `32395288682`](https://github.com/TaeeunKil/kafrust/actions/runs/32395288682). Qualify the current published secured/simultaneous-loss, concurrent-client, deploy/restart, and rollback profiles |
| Published `0.3.3` secured simultaneous broker-loss soak | Candidate for the tested current published SASL_SSL/SCRAM profile; a fresh external project resolved `kafrust 0.3.3` from crates.io with `tls`, ran 600 seconds against Kafka 4.3.1 with three replicated partitions, stopped brokers 1 and 2 simultaneously for ten seconds, reconciled 19,188,800 records, observed 283 operation errors, 6 failed requests, and 9 retries, and ended with `recovered=true` plus zero final in-flight/buffered records in [`Published Secure Multi-Broker Soak Smoke` run `32396241090`](https://github.com/TaeeunKil/kafrust/actions/runs/32396241090). Qualify repeated secured campaigns, unclean-election behavior, production SLOs, credential rotation, and rollback paths |
| Published `kafrust 0.3.5` secured simultaneous broker-loss soak | Candidate for the tested current published SASL_SSL/SCRAM profile; a fresh external Rust 1.81 project resolved `kafrust 0.3.5` from crates.io, ran the default 600-second Kafka 4.3.1 three-broker profile, stopped brokers 1 and 2 simultaneously for ten seconds, and verified recovery plus zero final in-flight/buffered records in [`Published Secure Multi-Broker Soak Smoke` run `32440677496`](https://github.com/TaeeunKil/kafrust/actions/runs/32440677496). The uploaded result artifact is retained; qualify repeated secured campaigns, unclean-election behavior, production SLOs, credential rotation, and rollback paths |
| Share acknowledgement response-loss reconciliation | Candidate for the injected current-source Kafka 4.3.1 path; a proxy dropped a `ShareAcknowledge` response and the client reconciled the ambiguous outcome with duplicate-safe accounting and clean shutdown in [`Live Kafka Share Acknowledgement Ambiguity` run `32449038941`](https://github.com/TaeeunKil/kafrust/actions/runs/32449038941). Qualify published secured Share, member churn, broader response-loss cases, and production SLOs |
...
| SASL/OAUTHBEARER | Candidate on the Kafka 3.7.2 built-in-validator and signed local OIDC/JWKS fixture profiles, including static/provider-backed authentication, flexible `SaslAuthenticate v2` initial authentication, and `v1` same-connection re-authentication; external provider-specific behavior, detached refresh workers, and production token/authorization policy remain open |
...
| Synchronous producer, buffered producer, manually assigned direct consumer, joined consumer group, Share consumer, Streams session, or broad Admin operations | Candidate through the optional `blocking` feature; `BlockingProducer`, `BlockingBufferedProducer`, `BlockingBufferedProducerHandle`, `BlockingConsumer`, `BlockingConsumerGroup`, `BlockingShareConsumer`, `BlockingStreamsGroupSession`, and `BlockingAdminClient` own a dedicated multi-thread Tokio runtime and reject nested-runtime calls. The currently exposed Admin surface, including Share Group State operations, is available synchronously; live qualification and a general alternate-runtime trait remain open |
| General non-Tokio runtime adapter | Blocked by design for now; the blocking feature isolates Tokio internally but does not expose a runtime-abstraction trait |
...
| Consumer-group listing | Candidate; high-level `list_groups` queries every advertised broker, negotiates ListGroups v4/v5 when available, deduplicates and sorts group listings, and preserves protocol type, coordinator ID, throttle time, state, type, API version, and broker errors. `list_groups_with_options` adds v4/v5 state/type filters; the low-level `Client::list_groups_v1` path remains available for exact legacy compatibility. The live smoke gate asserts v4 negotiation on Kafka 3.7.2 and v5 state/type-filter negotiation on Kafka 4.3.1, with both paths passing in [`Live Kafka Smoke` run `32382586220`](https://github.com/TaeeunKil/kafrust/actions/runs/32382586220). In-flight broker-stop recovery restarts the broker during the bounded reconnect loop and records `retries=7` on the Kafka 3.7.2 three-broker profile in [`Live Kafka Smoke` run `31616181960`](https://github.com/TaeeunKil/kafrust/actions/runs/31616181960), and active-group listing is verified from the published `0.2.28` artifact across classic and KIP-848 profiles in [`Published Crate Smoke` run `31737581786`](https://github.com/TaeeunKil/kafrust/actions/runs/31737581786); qualify target authorization and behavior while a coordinator broker remains unavailable |
