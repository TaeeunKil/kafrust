...
| SASL/OAUTHBEARER | Candidate only for the documented Kafka 3.7.2 unsecured-validator smoke; async token-provider callbacks and SASL Authenticate v1 session lifetime metadata exist, but automatic re-authentication and production OAuth/OIDC provider, token policy, and authorization behavior remain open |
