...
| Published `kafrust 0.2.28` secured simultaneous broker-loss soak | fresh external Cargo project with no workspace path dependency and the `tls` feature; Kafka 4.3.1 three-broker KRaft with SASL_SSL/SCRAM-SHA-256, three replicated partitions, `Acks::All`, and `min.insync.replicas=2` | 120-second, 1-KiB, batch-size-100 workload; stopped brokers 1 and 2 simultaneously after one third of the run, restarted both after 10 seconds, required replicated-ack record reconciliation, TLS/SCRAM retry recovery, and zero final queue gauges | [`Published Secure Multi-Broker Soak Smoke`, run `31750274774`](https://github.com/TaeeunKil/kafrust/actions/runs/31750274774) on 2026-08-14 | Passing; 2,704,200 records, 282 expected operation errors while two brokers were unavailable, 2 failed requests, 3 retries, `recovered=true`, and zero final in-flight/buffered records; one secured simultaneous-loss durability/availability profile, not unclean-election data-loss, production SLO, or service-canary evidence |
...
- The published performance workflow
  ...
  baseline, not a direct rust-rdkafka comparison or production SLO.
