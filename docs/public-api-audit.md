...
| Kafka Streams group | `StreamsGroupConfig`, `StreamsGroupSession`, `StreamsGroupSessionAssignment`, and `kafrust::streams` API 88 protocol types | Alpha, expert-level | This is a manual Kafka Streams group membership/heartbeat layer. It does not execute a Streams DSL or process tasks. The session now preserves the latest successful broker assignment snapshot, including the nullable status array, while member-epoch recovery, background heartbeat ownership, automatic assignment/task-state reconciliation, shutdown, and live Kafka Streams qualification remain before treating it as a compatibility claim. |
