...
| Consumer group | `ConsumerGroupConfig`, `ConsumerGroup`, `ConsumerGroupHeartbeat`, `RebalanceListener`, `RebalanceEvent`, `RebalancePhase` | Alpha, highest churn risk | The group path exposes synchronous before/after assignment callbacks. Keep callback ordering, cancellation, and background-heartbeat semantics explicit before `1.0`. |
