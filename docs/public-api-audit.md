...
| Errors | `Error`, `Result`, `BrokerErrorKind` | Alpha, stabilization candidate | Common startup configuration failures now use `Error::InvalidConfiguration { field, reason }`; consumer-group assignment deadlines use `Error::ConsumerGroupAssignmentTimeout { timeout_ms }`; keep Kafka broker error codes observable and preserve dedicated variants for missing bootstrap, TLS, SASL, and transaction-outcome failures. |
