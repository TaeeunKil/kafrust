...
| Kafka Streams group | `StreamsGroupConfig`, `StreamsGroupSession`, `StreamsGroupSessionAssignment`, and `kafrust::streams` API 88 protocol types | Alpha, expert-level | This is a manual Kafka Streams group membership/heartbeat layer. It does not execute a Streams DSL or process tasks. The session preserves the latest successful broker assignment snapshot, including the nullable status array, and the Kafka 4.3.1 live gate qualifies join, task-state heartbeat, repeated heartbeat, nullable task-offset omission, and graceful leave. Member-epoch recovery, background heartbeat ownership, automatic assignment/task-state reconciliation, complete application compatibility, and multi-member failure evidence remain before `1.0`. |
