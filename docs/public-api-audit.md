...
| Consumer group | `ConsumerGroupConfig`, `ConsumerGroup`, `ConsumerGroupHeartbeat`, `RebalanceListener`, `RebalanceEvent`, `RebalancePhase` | Alpha, highest churn risk | The group path exposes concrete and regex topic subscriptions plus synchronous before/after assignment callbacks. Keep metadata resolution, callback ordering, cancellation, and background-heartbeat semantics explicit before `1.0`. |
