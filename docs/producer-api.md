...
- `ProducerConfig::build_buffered` creates a `BufferedProducer` lifecycle skeleton with `flush`, `close`, and `is_closed`; record enqueue and delivery handles are still planned.
...
- Linger-based buffering enqueue and delivery are still planned as a separate buffered producer path.
