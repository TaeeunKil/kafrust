...
- `ProducerConfig::security_protocol` stores the Kafka security protocol for producer broker connections. `Plaintext` is the only implemented transport; TLS and SASL variants return `Unsupported` until M11 implements them.
