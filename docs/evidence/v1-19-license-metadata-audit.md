...
The dependency graph checker now resolves the locked workspace metadata and
rejects any package that has neither Cargo `license` nor `license_file`
metadata. The check covers the complete resolved graph rather than only the
two workspace crates and runs alongside the forbidden-package and version
checks in `scripts/check_v1_dependency_graph.py`.
