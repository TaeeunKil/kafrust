...
three-broker KRaft cluster, six partitions, two group members, an abrupt
connection drop of the second member, and an independent group ID per cycle.
Each cycle checked disjoint ownership, rejoin recovery, committed-offset
restoration, and zero final in-flight/buffered gauges.
...
Both summaries report `cycle_count=40`, `records_per_cycle=6`,
`loss_count=0`, `duplicate_count=0`, and final `in_flight=0` /
`buffered=0`. The classic log records cycles 1/40 through 40/40; the
consumer log does the same. The consumer run's cycle 1 completed at
03:15:55Z and cycle 40 at 03:48:41Z, explaining the longer KIP-848 wall time.
...
- Run the normal-member `LeaveGroup` matrix (the passing profiles above use
  abrupt `drop`).
