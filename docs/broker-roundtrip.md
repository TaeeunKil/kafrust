...
Latest manual live smoke: on 2026-07-23, GitHub Actions run `29990529966` passed the single-node plaintext path against Kafka 3.7.2 and 4.3.1, plus the Kafka 3.7.2 multi-broker, TLS, SASL_PLAINTEXT, and SASL_SSL SCRAM profiles. The plaintext matrix covered broker roundtrip, normal and idempotent single-record produce, batch, all four compression codecs, buffered producer, direct consumer, and consumer group paths. The TLS job additionally verified gzip, Snappy, LZ4, and Zstd batch production over a secured connection. The multi-broker job retained leader-stop producer and consumer failover coverage.
