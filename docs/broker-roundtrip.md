...
Latest manual live smoke: on 2026-07-23, GitHub Actions run `29986018854` passed against Kafka 3.7.2. The plaintext job covered the broker roundtrip test, `producer_send`, `producer_send_batch`, `producer_send_batch` with `KAFRUST_COMPRESSION=gzip`, `KAFRUST_COMPRESSION=snappy`, and `KAFRUST_COMPRESSION=lz4`, `producer_buffered`, `consumer_fetch`, and `consumer_group_poll` against `kafrust-smoke`. The multi-broker plaintext job covered the broker roundtrip test, `broker_roundtrip`, `producer_send`, `producer_send_batch`, `producer_send_batch` with `KAFRUST_COMPRESSION=gzip`, `KAFRUST_COMPRESSION=snappy`, and `KAFRUST_COMPRESSION=lz4`, `producer_buffered`, `consumer_fetch`, `consumer_group_poll`, `producer_failover`, and `consumer_failover` against a three-broker KRaft cluster with comma-separated `KAFRUST_BOOTSTRAP_SERVERS`, `KAFRUST_EXPECTED_BROKERS=3`, `KAFRUST_PARTITION=1` for single-record producer routing, and `KAFRUST_BATCH_PARTITIONS=0,1,2,3,4,5` for batch producer routing; it selected a partition led by the first broker, stopped that broker between two sends from the same producer instance and two fetches from the same consumer instance, then reran batch producer, direct consumer, and consumer group checks through the remaining brokers. The TLS job covered the broker roundtrip test, `broker_roundtrip`, `producer_send`, `producer_send_batch`, `producer_buffered`, `consumer_fetch`, and `consumer_group_poll` with `KAFRUST_SECURITY_PROTOCOL=tls`. The SASL_PLAINTEXT job covered the same broker roundtrip, producer, direct consumer, and consumer group paths with `KAFRUST_SECURITY_PROTOCOL=sasl_plaintext` and SASL/PLAIN credentials. The SASL_SSL SCRAM job covered those same paths with `KAFRUST_SECURITY_PROTOCOL=sasl_tls`, `KAFRUST_SASL_MECHANISM=scram-sha-256`, and a DER root certificate configured through `KAFRUST_TLS_ROOT_CERT_DER_PATH`.

The `Live Kafka Smoke` GitHub Actions workflow runs plaintext broker roundtrip, single-record producer, batch producer, gzip, Snappy, and LZ4 batch producers, buffered producer, producer failover, direct consumer, direct consumer failover, and consumer group checks against Kafka 3.7.2 Docker containers. It covers single-node plaintext, three-broker plaintext, TLS, SASL_PLAINTEXT, and SASL_SSL SCRAM profiles. The workflow is available through manual dispatch and a weekly schedule, so the default pull request CI remains broker-free.
