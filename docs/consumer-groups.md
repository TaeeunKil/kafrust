...
Under the hood, `ConsumerGroupConfig::join` discovers the coordinator, sends `JoinGroup v2`, computes range assignments if this member is the leader, sends `SyncGroup v2`, fetches committed offsets for assigned partitions, and builds a direct `Consumer` for fetching records. `ConsumerGroup::poll` sends a heartbeat before fetching assigned partitions, rejoins the group when the heartbeat reports a rebalance, stale generation, stale member ID, or stale coordinator, and `ConsumerGroup::commit_offsets` commits the current next offsets. If an offset commit reports a rejoinable generation, member, rebalance, or coordinator error, `commit_offsets` rejoins the group and returns the original broker error instead of retrying the old assignment offsets under the new generation. `ConsumerGroup::spawn_heartbeat_task` is an opt-in background heartbeat loop on a separate coordinator connection; the returned `ConsumerGroupHeartbeat` records the group ID, member ID, and generation ID it belongs to. Pass the handle to `ConsumerGroup::poll_with_heartbeat` to observe task completion before polling and rejoin when the task ended with a rejoinable group error. If the group already rejoined through another path, `poll_with_heartbeat` stops the stale heartbeat task before polling so the old generation does not keep sending heartbeats. Call `ConsumerGroupHeartbeat::try_wait` to observe early task completion without polling, and call `ConsumerGroupHeartbeat::stop` to shut it down and observe any broker error returned by the task.
